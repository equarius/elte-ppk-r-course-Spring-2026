...
filepath <- file.path(gsub("assignments","data",dirname(rstudioapi::getSourceEditorContext()$path)),
                 "assignment_3_dataset.xlsx")
rd<-read_excel(filepath,sheet=1) # load in data
unique(rd$sex)
rd[rd$sex=="woman","sex"] <- "female" #correct the wrong entries
crd <- rd %>% 
...
crdl$sex <- factor(as.character(crdl$sex), levels = c("1","2"), labels = c("1" = "female","2" = "male"))
```
...
```{r}
coodi <- cooks.distance(mc) #calculate cook's distance for influential outliers
coodi %>% 
  as.data.frame() %>% 
...
crdwo <- crd[-excl,]
#re-creating model for later analysis, consider running this also
...
#Other rationale would be to stick to the serum level, since it has been suggested by studies before as a strong associate of postoperative pain.
...
Y~pain~ = 1.47(intercept) - 0.23(age) + 0.31(male sex) - 0.03(STAI) + 0.09(pain catastrophization) - 0.14(mindfulness) + 0.58(saliva cortisol)
